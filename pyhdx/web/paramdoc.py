...
def param_format_basic(app, what, name, obj, options, lines):
    ...
    if what == "class" and isinstance(obj, param.parameterized.ParameterizedMetaclass):
        lines += [
            "..",
            DEDUPLICATION_TAG,
        ]  # Prevent numpydoc from mangling the docstring
        ...
        for cls in mro[:-1]:
            ...
            inherited.extend(
                [
                    "",
                    "    :class:`{module}.{name}`: {params}".format(
        ...
        for child in params:
            ...
            for m in members:
                ...
                if (
                    m[0][0] != "_"
                    and m[0] not in IGNORED_ATTRS
                    and not inspect.ismethod(m[1])
                    and not inspect.isfunction(m[1])
                    and m[1] is not None
                    and DEFAULT_VALUES.get(m[0]) != m[1]
                    and (m[0] != "label" or pobj.label != label)
                ):
