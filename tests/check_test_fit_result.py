...
fixed_result = csv_to_dataframe(test_data_dir / 'ecSecB_torch_fit_fixed.csv').rename(columns={'deltaG': 'dG'})
...
fixed_result = csv_to_dataframe(test_data_dir / 'ecSecB_torch_fit_epochs_20000_fixed.csv').rename(columns={'deltaG': 'dG'})
comparisons['single_20k'] = (new_result, fixed_result)

new_result = csv_to_dataframe(test_data_dir / 'output' / 'ecSecB_batch.csv')
fixed_result = csv_to_dataframe(test_data_dir / 'ecSecB_batch_fixed.csv').rename(columns={'deltaG': 'dG'})
comparisons['batch'] = (new_result, fixed_result)

new_result = csv_to_dataframe(test_data_dir / 'output' / 'ecSecB_batch_aligned.csv')
fixed_result = csv_to_dataframe(test_data_dir / 'ecSecB_batch_aligned_fixed.csv').rename(columns={'deltaG': 'dG'})
